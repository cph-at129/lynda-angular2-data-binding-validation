...
import {ControlGroup, Control, ControlArray, FormBuilder, RadioButtonState, Validators} from '@angular/common';
  ...
  private _buildForm(){
    this.orderSheetForm = this._formBuilder.group({
      ...
      sizes: this._formBuilder.group({
        sizeSmall: this._formBuilder.control(
          new RadioButtonState(false, 'small')
        ),
        sizeLarge: this._formBuilder.control(
          new RadioButtonState(false, 'large')
        )
      }, { validator: radioSetRequired }),
      specialtySandwich: this._formBuilder.control(null),
      breads: this._formBuilder.group({
        breadWheatRoll: this._formBuilder.control(
          new RadioButtonState(false, 'wheat roll')
        ),
        breadWhiteRoll: this._formBuilder.control(
          new RadioButtonState(false, 'white roll')
        ),
        breadSourdoughRoll: this._formBuilder.control(
          new RadioButtonState(false, 'sourdough roll')
        )
      }),
    ...
    customerNameControl.valueChanges.subscribe(value => {
      this.showWelcomeMessage = value.toLowerCase().trim() === 'terziev a.';
    });
